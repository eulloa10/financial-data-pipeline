...
resource "aws_s3_object" "glue_job_aggregate_month_fred_data" {
  ...
  source = "${path.root}/modules/glue/scripts/aggregate_jobs/aggregate_month_fred_data.py"
  etag = filemd5("${path.root}/modules/glue/scripts/aggregate_jobs/aggregate_month_fred_data.py")
}
