...
resource "aws_s3_object" "glue_job_extract_range_fred_api" {
  ...
  source = "${path.root}/modules/glue/scripts/extract_jobs/extract_range_fred_data.py"
  etag = filemd5("${path.root}/modules/glue/scripts/extract_jobs/extract_range_fred_data.py")
}
